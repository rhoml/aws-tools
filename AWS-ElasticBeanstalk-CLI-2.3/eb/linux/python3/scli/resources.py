...
class DevToolsMessage(object):
    GitCommandError = 'Unable to call Git. Install Git to deploy to Elastic Beanstalk.'
    GitRepoNotExist = 'Warning: Your directory has not been initialized as a Git repository. '\
        'To create a local Git repository, run "git init" and then re-run the "eb init" command.'
    InitError = 'Failed to update local Git repository setting.'
