...
def create_eb_local_dir():
    config_file.create_directory(os.getcwdu() + os.path.sep + EbLocalDir.Path)    
...
#            raise EBSCliException()
    ...
    def run_dev_tools_script(self):
        ...
        try:
            ...
            log.error(u'Dev Tools initialiation script report an error, because "{0}".'.format(ex))
            prompt.error(DevToolsMessage.InitError)
            raise
        
        except (OSError, IOError) as ex:
            log.error(u'Failed to call Dev Tools initialiation script, because "{0}".'.format(ex))
